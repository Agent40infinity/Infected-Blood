...
public class SpeedPerk : MonoBehaviour
{
    ...
    public int speedPerkPrice; // Price of the perk
    ...
    private void OnGUI()
    {
        if (showPerkMachineGUI) // If the player is within the range of the perk machine this menu will populate allowing the player to purchase it
        {
            ...
            (Screen.height / 1.4f), 800, 100), "Press key << E >> to buy perk $" + speedPerkPrice); // The text and price for the label
        }
    }
    ...
    private void OnTriggerStay(Collider other)
    {
        if (other.tag == "Player")
        {
            ...
            //if (Input.GetKeyDown ("e") && GameManager.playerCash >= speedPerkPrice && showPerkMachineGUI)
            ...
            //{
            //    GameManager.playerCash -= speedPerkPrice; // Cash is deducted and the player gets the perk
            //    GameManager.playerHasSpeedPerk = true; // Telling the game manager to make it true so they player cant buy it twice
            ...
            // other.SendMessage("GivePlayerPerk", "speedPerk"); Dispences the health perk to the player
            //}
        }
    }
     ...
     * if(whichPerk == "speedPerk") (this if statement check if the perk is the health perk and applies the extra stats)
     * {
